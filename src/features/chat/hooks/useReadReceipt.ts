...
export const useReadReceipt = (conversationId: string | null, messages: Message[]) => {
      ...
      if (document.hasFocus()) {
        ...
        queryClient.setQueriesData({ queryKey: ['conversations'] }, (old: any) => {
          ...
          const updateList = (list: any[]) => list.map(c => 
            c.id === conversationId 
              ? { ...c, currentParticipant: { ...c.currentParticipant, lastReadMessageId: lastMsg.id } }
              : c
          );
          if (Array.isArray(old)) return updateList(old);
          if (old.data && Array.isArray(old.data)) return { ...old, data: updateList(old.data) };
          if (old.items && Array.isArray(old.items)) return { ...old, items: updateList(old.items) };
          return old;
        });
      }
    };
